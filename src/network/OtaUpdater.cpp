...
esp_err_t http_client_set_header_cb(esp_http_client_handle_t http_client) {
  return esp_http_client_set_header(http_client, "User-Agent", "RadioInk-ESP32-" RADIOINK_VERSION);
}
}  // namespace

OtaUpdater::OtaUpdaterError OtaUpdater::checkForUpdate() {
  LOG_DBG("OTA", "Checking for update (current: %s)", RADIOINK_VERSION);
...
bool OtaUpdater::isUpdateNewer() const {
  if (!updateAvailable || latestVersion.empty() || latestVersion == RADIOINK_VERSION) {
    return false;
  }
  ...
  const auto currentVersion = RADIOINK_VERSION;
